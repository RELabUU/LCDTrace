...
def preprocess(text):
    ...
    #no_interpunction = lower_case_string.translate(None, string.punctuation)
...
def preprocessJiraData(cleanDataFrame, preprocessComments, startTime):
    ...
    jira_summaries = cleanDataFrame['Summary'].apply(lambda x: preprocess(x))
    ...
    jira_descriptions = cleanDataFrame['Description'].apply(lambda x: preprocess(x))
    ...
    if (preprocessComments == True):
        jira_comments = cleanDataFrame['Comments'].apply(lambda x: preprocess(x))
...
def cleanJiraData(dataFrame, cleanComments, commentAmount):
    ...
    if (cleanComments == True):
        ...
        cleanedAndProcessedJiraData = preprocessJiraData(jira_issues_subset, preprocessComments = True, startTime = startTime)
        ...
        cleanedAndProcessedJiraData = preprocessJiraData(jira_issues_subset, preprocessComments = False, startTime = startTime)
        return(cleanedAndProcessedJiraData)
